...
from ..parser.chapter_parser import ChapterChunk
from ..summarizer.summary_generator import SummaryResult
...
class CheckpointManager:
    ...
    def load_checkpoint(self, session_id: str) -> tuple[CheckpointState, List[ChapterChunk], List[SummaryResult], Optional[List[SummaryResult]]]:
        """加载检查点"""
        ...
        if state.polished_summaries_file and Path(state.polished_summaries_file).exists():
                ...
                if state_file.exists():
                    try:
                        ...
                        checkpoints.append({
                            ...
                            'failed_count': len(state_dict.get('failed_chapter_ids', []))
                        })
            ...
            if session_dir.is_dir():
                ...
                if state_file.exists():
                    try:
                        ...
                        created_time = datetime.fromisoformat(state_dict['created_at']).timestamp()
...
def print_checkpoints_table(checkpoints: List[Dict[str, Any]]) -> None:
    ...
    print("=" * 120)
    print(f"{'会话ID':<12} {'输入目录':<25} {'压缩':<4} {'阶段':<15} {'进度':<10} {'失败':<4} {'更新时间':<19}")
    print("-" * 120)
    
    for cp in checkpoints:
        print(f"{cp['session_id']:<12} {cp['input_dir'][-24:]:<25} "
