...
from .checkpoint_manager import CheckpointManager, CheckpointState, SummaryResult, print_checkpoints_table
...
class SummaryEngine:
    ...
    def __init__(self, config: UnifiedConfig):
        self.config = config
        self.checkpoint_manager = CheckpointManager(Path(config.processing.checkpoint_dir)) if config.processing.enable_checkpoint else None
    ...
    async def summarize(
        ...
        if resume_session:
            ...
            await self._new_summarize(input_dir, output_path, compression_level)
    ...
    async def _new_summarize(
        ...
        summaries = await self._generate_summaries_with_checkpoint(
            chapter_chunks, compression_level, state
        )
        ...
        if self.config.polish.enabled and summaries:
            ...
            polished_summaries = await self._polish_summaries_with_checkpoint(
                summaries, compression_level, state
            )
            print(f"打磨完成: {len(polished_summaries)} 个精化总结")
            final_summaries = polished_summaries
        
        # 5. 完成处理
        await self._finalize_summary(final_summaries, state, chapter_chunks, output_path, compression_level)
    ...
    async def _resume_summarize(self, session_id: str) -> None:
        ...
        if state.current_stage == "initial_summary":
            ...
            print("\n继续生成初步总结...")
            summaries = await self._generate_summaries_with_checkpoint(
                chapters, state.compression_level, state, existing_summaries=summaries
            )
            ...
            if self.config.polish.enabled and summaries:
                ...
                polished_summaries = await self._polish_summaries_with_checkpoint(
                    summaries, state.compression_level, state
                )
                final_summaries = polished_summaries
            else:
                final_summaries = summaries
            ...
            print("\n继续二次打磨...")
            polished_summaries = await self._polish_summaries_with_checkpoint(
                summaries, state.compression_level, state, existing_polished=polished_summaries
            )
            final_summaries = polished_summaries
        ...
        await self._finalize_summary(
            final_summaries, state, chapters, 
            Path(state.output_path), state.compression_level
        )
                    ...
                    try:
                        ...
                        if state:
                            ...
                            if "RATE_LIMIT_ERROR" in str(e) or "rate limit" in str(e).lower():
                                ...
                                print(f"   可以稍后运行以下命令恢复: mineru-summarizer --resume {state.session_id}")
                                break
                        else:
                            print(f"\n❌ 章节处理失败: {chunk.chapter_title}")
                            print(f"   错误: {str(e)}")
    ...
    async def _polish_summaries_with_checkpoint(
        ...
        polish_config.processing.max_concurrent = max(1, int(self.config.processing.max_concurrent * self.config.polish.concurrent_ratio))
        ...
        async with backend:
            with tqdm(total=len(pending_summaries), desc="打磨总结", initial=len(completed_ids)) as pbar:
                for summary in pending_summaries:
                    try:
                        # 构建打磨提示词
                        polish_prompt = self._build_polish_prompt(summary, compression_level)
    ...
    def _build_summary_prompt(self, chunk: DocumentChunk, compression_config: Dict[str, str], compression_level: int) -> str:
        """构建总结提示词"""
...
5. 如果原文中有图片链接（格式如![](images/xxx.jpg)），请中保留这些图片链接，如果文中有对图片的相关描述可以简要提及
6. 专有技术名词保持英文原文，不要翻译，包括但不限于：
...
请用中文回答，但保持上述英文技术术语："""
    ...
    def _build_polish_prompt(self, summary: SummaryResult, compression_level: int) -> str:
        """构建打磨提示词"""
...
请输出打磨后的总结，直接给出最终结果，不要包含说明性文字："""
    ...
    def _calculate_overall_stats(self, summaries: List[SummaryResult]) -> Dict[str, Any]:
        """计算整体统计信息"""
        if not summaries:
            return {}
        
        total_original_tokens = sum(s.metadata.get('original_token_count', 0) for s in summaries)
