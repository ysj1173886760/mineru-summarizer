...
class ChapterChunk:
    ...
    chapter_title: str
    chapter_level: int
    sub_sections: List[str]  # 子章节标题列表


class ChapterBasedParser:
    """基于大章节的文档解析器"""
    
    def __init__(self, max_tokens_per_chapter: int = 10000):
    ...
    def parse_markdown_file(self, file_path: Path) -> List[ChapterChunk]:
        """按大章节解析Markdown文件"""
    ...
    def _split_large_chapter(self, chunk, chapter_index: int) -> List[ChapterChunk]:
        """分割过大的章节"""
        # 创建包含二级标题的分割器
    ...
    def get_chapter_statistics(self, chunks: List[ChapterChunk]) -> Dict[str, Any]:
        """获取章节统计信息"""
        stats = {
    ...
    def print_chapter_info(self, chunks: List[ChapterChunk]) -> None:
        """打印章节信息"""
        stats = self.get_chapter_statistics(chunks)
...
def main():
    """测试函数"""
    parser = ChapterBasedParser(max_tokens_per_chapter=8000)
    
    # 测试解析GFM_SURVEY的full.md
    md_file = Path("../GFM_SURVEY/full.md")
    if md_file.exists():
        print(f"📄 使用大章节分片解析: {md_file}")
        chunks = parser.parse_markdown_file(md_file)
        
        parser.print_chapter_info(chunks)
        ...
        for i, chunk in enumerate(chunks[:2]):
            ...
            print(f"   内容预览: {chunk.content[:300]}...")
    else:
